...
//🚨    val totalElecPriceOver500Sequence = ?
    val totalElecPriceOver500Sequence = products.asSequence().filter { it.price > 500 && it.category == "Electronics" }.map { it.price }.fold(0.0) { acc, i -> acc + i }
...
//🚨    println("ผลรวมราคาสินค้า Electronics ที่ราคา > 500 บาท: $totalElecPriceOver500Sequence บาท")
    ...
    println("--------------------------------------------------")
    println("กลุ่มสินค้าที่ราคาไม่เกิน 1000 บาท")
    var productMin = products.filter { it.price < 1000}.forEach { println("\t- ${it.name} (${it.price}บาท)") }
    println("กลุ่มสินค้าที่ราคาไม่เกิน 1000-9999 บาท")
    var productMid = products.filter { it.price > 1000 && it.price < 9999}.forEach { println("\t- ${it.name} (${it.price}บาท)") }
